    ...
    public override void Execute(Miner miner)
    {
        if (miner.m_Thirst > 7 || miner.m_Fatigue > 7 || miner.m_GoldCarried == 10) 
        {
            if (miner.m_StateMachine.m_CurrentState.stateName != StateName.VisitBankAndDepositGold)
            {
                ...
                else if (miner.m_StateMachine.m_CurrentState.stateName != StateName.EatStew && miner.m_StateMachine.m_CurrentState.stateName != StateName.GoHomeAndSleepTilRested && miner.m_StateMachine.m_CurrentState.stateName != StateName.QuenchThirst && miner.m_StateMachine.m_CurrentState.stateName != StateName.EatStew)  
                {
                    if (miner.m_Thirst > 7)
                    {
