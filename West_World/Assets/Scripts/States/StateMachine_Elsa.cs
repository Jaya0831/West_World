...
public class StateMachine_Elsa : MonoBehaviour
{
    ...
    {
        Debug.Log("HandleMessage: Elsa's StateMachine");
        if (e_CurrentState.OnMessage(owner.GetComponent<Elsa>(), msg))
        {
            return true;
        }
        if (e_GlobalState.OnMessage(owner.GetComponent<Elsa>(), msg))
        {
            return true;
        }
        return false;
