...
public class Grid : MonoBehaviour
{
    ...
    /// <summary>
    /// 地点位置信息（Location_Type 的前8个）
    /// </summary>
    public List<Vector3>[] objectInf = new List<Vector3>[8];
    ...
    public List<Vector3> FindWay(Vector3 now, Vector3 destination)
    {
        ...
        // HACK:扩展性
        if ((int)grid[dest_vec2.x, dest_vec2.y].type < 7)
        {
            grid[dest_vec2.x, dest_vec2.y].status = Node.Status.undiscovered;
        }
        if ((int)grid[now_vec2.x, now_vec2.y].type < 7)
        {
        ...
        for (int i = left; i <= right; i++)
        {
            for (int j = down; j <= up; j++)
            {
                if (sprite_obstacle.Exists(sp => sp.Equals(mytilemap.GetSprite(new Vector3Int(i, j, 0)))))
                {
                    ...
                    else if (mytilemap.GetSprite(new Vector3Int(i, j, 0)) == sprite_obstacle[6])
                    {
                        objectInf[(int)Node.Location_Type.Stove].Add(FromVectorToPosition(new Vector2Int(i - left, j - down)));
                        grid[i - left, j - down].type = Node.Location_Type.Stove;
                    }
                    else if (mytilemap.GetSprite(new Vector3Int(i, j, 0)) == sprite_obstacle[7])
                    {
                        objectInf[(int)Node.Location_Type.Table].Add(FromVectorToPosition(new Vector2Int(i - left, j - down)));
                        grid[i - left, j - down].type = Node.Location_Type.Table;
                    }
                    else
                    {
